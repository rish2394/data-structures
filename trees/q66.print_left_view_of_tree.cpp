...
/* Recursive Method */
void printLeftViewRecursiveUtil(tree root, int level, int &k) {
	if(!root) {
		return;
  }
	if(level > k) {
	...
	printLeftViewRecursiveUtil(root->left, level+1, k);
	printLeftViewRecursiveUtil(root->right, level+1, k);
}
void printLeftViewRecursive(tree root) {
	if(!root) {
		return;
  }
	int k = INT_MIN;
	printLeftViewRecursiveUtil(root, 1, k);
}
